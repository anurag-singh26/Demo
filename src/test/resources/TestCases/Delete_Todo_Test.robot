...
${GRAPHQL_DELETE}    {"query": "mutation { delete_todos(where: {id: {_eq: 457224}}) { affected_rows returning { title } } }"}
...
# Test Case 1: GraphQL Mutation to Delete Todo
Delete Todo Test
    [Tags]  Regression
    [Documentation]    Test GraphQL mutation to delete an existing todo item and validate the response.
    ...
    ...  mutation {
    ...      delete_todos(where: {id: {_eq: 457380}}) {
    ...        affected_rows
    ...        returning {
    ...          title
    ...        }
    ...
    # Send GraphQL mutation to delete the todo item
...
# Test Case 2: Invalid Token Test for Delete
Invalid Token Delete Test
    [Documentation]    Test GraphQL mutation with an invalid token and check for unauthorized error when deleting a todo.

    # Create session with invalid Authorization Header
    ...
    # Send GraphQL mutation to delete the todo item with invalid token
    ${response}=    POST Request    graphql_session    ${BASE_URL}    data=${body}    headers=${header}
    ...
    # Assert that the response contains the access-denied error message
    ${response_body}=    Convert to String    ${response.content}
    Should Contain    ${response_body}    unauthorized

# Test Case 3: Missing Token Test for Delete
Missing Token Delete Test
    [Documentation]    Test GraphQL mutation without the Authorization token.
    ...
    # Send GraphQL mutation to delete the todo item without token
    ${response}=    POST Request    graphql_session    ${BASE_URL}    data=${body}    headers=${header}
