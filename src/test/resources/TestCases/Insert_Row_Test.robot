...
${GRAPHQL_MUTATION}    mutation ($todo: todos_insert_input!) { insert_todos(objects: [$todo]) { affected_rows returning { id created_at title } } }
...
# Test Case 1: Valid Insert Data Test
Insert Row Test
    [Tags]  Regression
    [Documentation]    Test GraphQL mutation to insert a new todo item and validate its insertion.
    ...
    # Send GraphQL mutation to insert a new todo item
...
Invalid Token Test
    ...
    ${json_string}=    Catenate
        ...  mutation {
        ...      insert_todos(objects: [{title: "Invalid Token Test"}]) {
        ...          affected_rows
        ...          returning {
        ...              id
        ...              created_at
...
Invalid Query Test
    ...
    ${json_string}=    Catenate
        ...  mutation {
        ...      insert_todos(objects: [{wrong_field: "invalid"}]) {
        ...          affected_rows
        ...          returning {
        ...              id
        ...              created_at
    ...
    # Send GraphQL mutation with invalid query
    ${response}=    POST Request    graphql_session    ${BASE_URL}    data=${body}    headers=${header}
...
Missing Token Test
    ...
    ${json_string}=    Catenate
        ...  mutation {
        ...      insert_todos(objects: [{title: "Missing Token Test"}]) {
        ...          affected_rows
        ...          returning {
        ...              id
        ...              created_at
...
# Test Case 5: Empty Query Test
Empty Query Test
    [Tags]  Regression

    [Documentation]    Test GraphQL mutation with an empty query body.

    # Create session with Authorization Header for Bearer Token
    ${headers}=    Create Dictionary    Authorization=${TOKEN}    Content-Type=application/json
    Create Session    graphql_session    ${BASE_URL}    headers=${headers}

    # Create an empty GraphQL mutation string
    ${json_string}=    Catenate
        ...  mutation {
    ...
    # Send GraphQL mutation with empty query
    ${response}=    POST Request    graphql_session    ${BASE_URL}    data=${body}    headers=${header}
    ...
    # Status code assertion for empty query
    Should Be Equal As Numbers    ${response.status_code}    200

    ${response_body}=    Convert to String    ${response.content}
    Should Contain    ${response_body}    affected_rows
